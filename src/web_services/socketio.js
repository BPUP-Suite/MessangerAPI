...
io.use(async (socket, next) => {
  try {
    ...
    if (activeSessions.has(session_id)) {
      ...
      if (existingSocket) {
        logger.warn(
          `[IO] Another socket already connected with session ${session_id}. Disconnecting previous socket.`
        );
    ...
    logger.debug(
      `[IO] IO authentication successful: user_id ${user_id}, added to group`
    );
...
function send_messages_to_recipients(
  recipient_list,
  message_data,
  sender_socket_id
) {
...
function join_comms(socket_id, room_id, comms_id) {
  ...
  if (existingRoom) {
    debug(
      "join_comms",
      "FUNCTION",
      "User " + socket.user_id + " is already in comms",
      existingRoom
    );
  ...
  debug(
    "join_comms",
    "FUNCTION",
    "User " + socket.user_id + " joined comms with comms_id" + comms_id,
    room_id
  );
...
function leave_comms(socket_id) {
  ...
  if (existingRoom) {
    ...
    if (
      ...
      socketData.active_screen_shares.length > 0
    ) {
      ...
      debug(
        "leave_comms",
        "FUNCTION",
        `Destroyed ${activeScreenShares.length} active screen shares for user ${socket.user_id}`
      );
    }
  ...
  debug(
    "leave_comms",
    "FUNCTION",
    "User " + socket.user_id + " is not in any room",
    ""
  );
...
function is_already_in_room(socket) {
  try {
    ...
    const chat_rooms = rooms.filter((room) => {
      return room !== socket.id && room !== socket.user_id;
    });
    ...
    if (chat_rooms.length > 0) {
      debug(
        "is_already_in_room",
        "FUNCTION",
        `User ${socket.user_id} is already in rooms:`,
    ...
    debug(
      "is_already_in_room",
      "FUNCTION",
      `User ${socket.user_id} is not in any other room`
    );
    return null;
  } catch (err) {
    error(
      "is_already_in_room",
      "FUNCTION",
      `Error checking rooms for user ${socket.user_id}. ` + err
    );
    return null;
  }
}
...
function send_to_comms_id(comms_id, data, type) {
  ...
  if (!socket_id) {
    logger.error(
      `[IO] Error getting socket id for comms ${comms_id} [DEPRECATED]`
    );
    return;
  }
...
function send_to_all_except_sender(
  recipient_list,
  data,
  type,
  sender_socket_id
...
async function get_users_info_room(chat_id) {
  try {
    ...
    error(
      "getUserIdsInRoom",
      "FUNCTION",
      `Error getting user_ids from room ${chat_id}` + err
    );
    return [[], [], [], []];
  }
}
...
function get_socket_id_from_comms_id(comms_id) {
  ...
  const socket = Array.from(activeSockets.values()).find(
    (socket) => socket.comms_id === comms_id
  );
  if (!socket) {
...
function stop_screen_share(socket_id, chat_id, screen_share_uuid, members) {
  ...
  if (socketData) {
    comms_id = socketData.comms_id;
    socketData.active_screen_shares = socketData.active_screen_shares.filter(
      (id) => id !== screen_share_uuid
    );
    activeSockets.set(socket_id, socketData);
  }
  const data = {
