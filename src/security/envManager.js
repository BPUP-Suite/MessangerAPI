...
function readVariable(name, needed) {
  ...
  if (needed && !VARIABLE) {
    console.log(
      `[ENV MANAGER] ${name} must be provided (${name} variable) (check .env file)`
    );
    throw new Error(
      `${name} must be provided (${name} variable) (check .env file)`
    );
  } else if (!VARIABLE) {
...
function readJWTSecret() {
  const secret = readVariable("JWT_SECRET", true);
  if (secret.length < 32) {
...
function readSMTPServer() {
  return readVariable("SMTP_SERVER", true);
}
function readSMTPPort() {
  return readVariable("SMTP_PORT", true) || "465"; // Default to 465
}
function readSMTPUser() {
  return readVariable("SMTP_USER", true);
}
function readSMTPPassword() {
  return readVariable("SMTP_PASSWORD", true);
}
