...
function readSalt() {
    try {
        logger.debug(`[FILE MANAGER] Reading salt from ${SALT_PATH}`);
...
function writeSalt(salt) {
    logger.debug(`[FILE MANAGER] Writing salt to ${SALT_PATH}`);
...
function readSessionKey() {
    try {
        logger.debug(`[FILE MANAGER] Reading session_key from ${SESSION_PATH}`);
...
function writeSessionKey(session_key) {
    logger.debug(`[FILE MANAGER] Writing session_key to ${SESSION_PATH}`);
...
function getSalt(){
    ...
    if (!SALT) {
        ...
        writeSalt(SALT);
        logger.debug(`[FILE MANAGER]Salt generated: ${SALT}`);
    }else{
        logger.debug(`[FILE MANAGER] Salt read: ${SALT}`);
    }
...
function getSessionKey(){
    ...
    if (!SESSION) {
        logger.debug(`[FILE MANAGER] Session_key not found in ${SESSION_PATH}`);	
        // If the SESSION KEY does not exist, generate a new one
        SESSION = crypto.randomBytes(32).toString('hex');
        writeSessionKey(SESSION);
        logger.debug(`[FILE MANAGER] Session_key generated: ${SESSION}`);
    }else{
        logger.debug(`[FILE MANAGER] Session_key read: ${SESSION}`);
    }
